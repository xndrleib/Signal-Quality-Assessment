...
def segment_signal(
    ...
    :param step: Number of samples to shift the window for each iteration (non-overlapping windows).
    :param overlap: Fraction of overlap between consecutive windows (0 to 1). Ignored if `step` is provided.
    ...
    if step is not None:
        ...
        logger.debug(f"Segmented with step={step}, total segments={len(segments)}")
        ...
        logger.debug(f"Segmented with overlap={overlap:.2f}, total segments={len(segments)}")
    else:
...
def time_to_freq_transform(
    ...
    :return: (yf, freqs) where yf is amplitude (dB or linear), freqs is array of frequencies.
    """
    if data.ndim != 1:
...
def fft_segment(
    ...
    logger.debug(f"FFT computed on {num_segments} segments, resulting shape={fft_segments.shape}")
    return fft_segments, freqs
