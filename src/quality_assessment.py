...
def weighted_average_metric(metrics: np.ndarray, weights: np.ndarray) -> float:
    ...
    if len(metrics) == 0 or len(weights) == 0 or np.sum(weights) == 0:
        logger.warning(
            "weighted_average_metric called with empty \
                metrics or zero-sum weights.")
        return 0.0
...
def dominant_frequency_metric(
    ...
    :return: Ratio of amplitude near target freq to \
    ...
    if not np.any(mask_target):
        logger.info(
            f"No frequency bins within {target_freq} ± \
                {freq_tolerance} Hz found.")
        return 0.0
    ...
    logger.debug(
        f"dominant_frequency_metric -> target_amp={target_amp:.3f}, \
            rest_amp={rest_amp:.3f}, ratio={ratio:.3f}")
    return float(ratio)
